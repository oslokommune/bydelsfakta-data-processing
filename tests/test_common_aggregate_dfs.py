...
def _sort_dfs(dfs):
    ...
    def _sort_df(df):
        ...
        df = df[col_order]
        sort_order = [
            col for col in ["delbydelid", "district", "date"] if col in used_basic_cols
        ]
...
class Tester(unittest.TestCase):
    ...
    def test__one_aggregation(self):
        ...
        aggregation = {"agg_func": "sum", "data_points": "inhabitants"}
        groupby = ["date"]
    ...
    def test__aggregate_district(self):
        ...
        aggregations = [
            {"agg_func": "sum", "data_points": "inhabitants"},
            {
                "agg_func": "wmean",
                "data_points": "mean_income",
                "data_weights": "inhabitants",
        ...
        df_act, df_exp = _sort_dfs(
            [df_act, df_agg_districts[df_agg_districts["district"] == DISTRICT].copy()]
        )
    ...
    def test_add_ratios(self):
        ...
        df = aggregate_dfs.add_ratios(
            df,
            ["mean_income", "double_mean_income"],
            ["mean_income", "double_mean_income"],
        )

        self.assertListEqual(
            [1 / 3 for i in range(len(df))], list(df["mean_income_ratio"])
        )
        self.assertListEqual(
