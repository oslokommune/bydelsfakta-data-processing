...
value_columns = ['enfamiliehusholdninger_med_voksne_barn', 'flerfamiliehusholdninger_med_smaa_barn',
                 'flerfamiliehusholdninger_med_store_barn', 'flerfamiliehusholdninger_uten_barn_0_til_17_aar',
                 'mor_eller_far_med_smaa_barn', 'mor_eller_far_med_store_barn', 'par_med_smaa_barn',
                 'par_med_store_barn', 'par_uten_hjemmeboende_barn']
...
def process(source):
    ...
    household_pivot = pd.pivot_table(meta, index=column_names.default_groupby_columns(),
                                     columns=['barn_i_husholdningen'], values=['total'])
    ...
    household_pivot["with_children"] = household_pivot['1 barn i HH'] + household_pivot['2 barn i HH'] + \
    ...
    househoulds = household_pivot[column_names.default_groupby_columns() + ["no_children", "with_children"]]
    ...
    aggregated = agg.add_ratios(aggregated, data_points=['no_children', 'single_adult', 'with_children'], ratio_of=['no_children', 'single_adult', 'with_children'])
    return aggregated


def write(output: Output, output_key):
    aws.write_to_intermediate(output_list=output.generate_output(), output_key=output_key)


if __name__ == "__main__":
    handle({
        "input": {
            "husholdninger-med-barn": util.get_latest_edition_of("husholdninger-med-barn")
        },
