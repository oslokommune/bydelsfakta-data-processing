...
def start(sysselsatte_key, befolkning_key):
    sysselsatte_raw = common_aws.read_from_s3(s3_key=sysselsatte_key, date_column="År")
    ...
    sysselsatte_df[data_point] = sysselsatte_df[data_point].apply(
        string_with_whitepace_to_int
    )
    sysselsatte_befolkning_df = aggregator.merge_dfs(
        sysselsatte_df, befolkning_df, how="outer"
    )

    aggregated_df = aggregator.aggregate_from_subdistricts(
        sysselsatte_befolkning_df, aggregations=_aggregations([data_point, total])
    )

    input_df = aggregator.add_ratios(aggregated_df, [data_point], ratio_of=[total])

    sysselsatte_historic = generate_output_list(
        *transform.historic(input_df), template="b", data_points=[data_point]
    )
    sysselsatte_status = generate_output_list(
        *transform.status(input_df), template="a", data_points=[data_point]
    )
    ...
    _write_to_intermediate(
        ...
        historic_version_id,
        historic_edition_id,
        sysselsatte_historic,
    )
    _write_to_intermediate(
        status_dataset_id, status_version_id, status_edition_id, sysselsatte_status
    )
