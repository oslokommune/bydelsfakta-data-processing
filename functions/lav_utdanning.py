...
def start(bucket, key):
    low_education_raw = common_aws.read_from_s3(s3_key=key, date_column="År")

    data_points = ["low_education"]

    input_df = generate_input_df(low_education_raw.copy())

    low_education_historic = generate_output_list(
        *transform.historic(input_df), template="b", data_points=data_points
    )
    low_education_status = generate_output_list(
    ...
    _write_to_intermediate(
        ...
        historic_version_id,
        historic_edition_id,
        low_education_historic,
    )
    _write_to_intermediate(
        status_dataset_id, status_version_id, status_edition_id, low_education_status
    )
...
def generate_input_df(df):
    ...
    df_aggregated = aggregator.aggregate_from_subdistricts(
        with_district, _aggregations(value_cols)
    )

    data_point = "low_education"
    df_aggregated[data_point] = (
        df_aggregated["Ingen utdanning/Uoppgitt utdanning"]
        + df_aggregated["Grunnskole"]
    )
