...
def data_processing(df):
    ...
    value_labels = [
        "Antall personer, fullført i løpet av 5 år",
    ...
    output_data["levekar_vgs_status"] = common.transform_output.generate_output_list(
        status_agg, "a", ["Antall personer, ikke fullført i løpet av 5 år"]
    )
    output_data["levekar_vgs_historisk"] = common.transform_output.generate_output_list(
        historic_agg, "b", ["Antall personer, ikke fullført i løpet av 5 år"]
    )
...
def handler(event, context):
    ...
    output_key = (
        f"processed/green/{dataset_ID}/version={ver_ID}/edition={edition_ID}/"
    )  # Temporary until the pipeline can handle data from intermediate
