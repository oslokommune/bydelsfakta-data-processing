...
def start(bucket, key):
    ...
    base = (
        original.groupby(["district", "delbydelid", "date", "Alder", "Kjønn"])["value"]
        .sum()
        .reset_index()
    )
    base = (
        base.pivot_table(
            values="value",
            index=["district", "delbydelid", "date", "Alder"],
            columns="Kjønn",
            fill_value=0,
        )
        .rename(columns={1: "mann", 2: "kvinne"})
        .reset_index()
    )

    base = (
        base.groupby(["district", "delbydelid", "date"])
        .apply(lambda x: _ensure_sequential_age(x.name, x))
    ...
    district = (
        base.groupby(["district", "date", "Alder"])["mann", "kvinne", "value"]
        .sum()
        .reset_index()
    )
    ...
    oslo = (
        base.groupby(["date", "Alder"])["mann", "kvinne", "value"].sum().reset_index()
    )
...
def to_quantile(df, quantile):
    ...
    subdistrict = (
        df.groupby(["district", "delbydelid", "date"]).apply(percentile).reset_index()
    )
...
def to_json(df, series):
    ...
    for name, group in df.groupby(["district", "date"]):
        if name[0] != "00":
            data = [
                geo_to_json(subgroup, series, name[1], sub, avgRow=(sub == name[0]))
    ...
    for sub, subgroup in df[df["delbydelid"].isna()].groupby(["district", "date"]):
        if sub[0] in ["16", "17", "99"]:
            continue
        data = [
            geo_to_json(subgroup, series, sub[1], sub[0], totalRow=(sub[0] == "00"))
        ]
...
def create_ds(df, dataset_id):
    ...
    common.aws.write_to_intermediate(
        output_key=output_key, output_list=jsonl, heading=heading, series=series
    )
    return output_key
