...
class Output:
    ...
    def _generate_district(self, district_id, metadata):
        df = self.df
        district = df[df[self.column_names.district_id] == district_id]  # get data from the current district
        sub_districts = district[
            self.column_names.sub_district_id].dropna().unique()  # get all sub districts in current district
    ...
    def _generate_data(self, df,
        ...
        values = self.template.values(df=df, series=self.values, column_names=self.column_names)
