...
def _check_city_level_totals(df_with_city_level, aggregations):
    ...
    sum_fields = [agg['data_points'] for agg in aggregations if agg['agg_func'] == 'sum']

    if len(sum_fields) > 0:
        df_agg = df_with_city_level[['date', 'city_level', *sum_fields]]\
            .groupby(['date', 'city_level'], as_index=False).sum()
        ...
        for year in list(df_agg['date'].unique()):
            ...
            for sum_field in sum_fields:
                ...
                if not all([x == col_content[0] for x in col_content]):
                    print(df_agg_year)
                    raise ValueError(f'The totals are not equal for all city levels during {year}, '
